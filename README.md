...
- Frigate 0.18.0-beta2 with separate `hailo8l` and `hailo10h` detector plugins.
...
- Frigate image builds from `ghcr.io/blakeblackshear/frigate:0.18.0-beta2-standard-arm64`.
- Frigate reports a `0.18.0-beta2` version on `/api/version` after startup.
...
## Frigate 0.18.0 Beta 2 Upgrade Notes

This image uses the upstream `v0.18.0-beta2` release. Beta 2 adds security fixes for database REGEXP evaluation and JWT expiry/logout handling, plus fixes for runtime camera toggles, ONVIF, autotracking, GenAI, semantic search, exports, and the Frigate UI.

The 0.18 upgrade still includes breaking changes that may require attention after the automatic configuration migration:

- FFmpeg is now version 8. Update go2rtc hardware-accelerated transcode settings if applicable.
- Snapshot files are clean WebP images; annotated JPEG files and `snapshots.clean_copy` are no longer stored on disk.
- Zones and masks now use `enabled` and `friendly_name` fields.
...
Review the upstream [Frigate 0.18.0 Beta 2 release notes](https://github.com/blakeblackshear/frigate/releases/tag/v0.18.0-beta2) and take a backup before upgrading a production installation.
...
- Tags: `0.18.0-beta2`, `latest`, `0.18.0-beta1`, `0.17.2`, `local`
- Link: https://hub.docker.com/r/msorenss79/hailo-frigate-h10

The `latest` tag currently points at the 0.18.0-beta2 image. Use the versioned tag if you want an explicit beta pin, or `0.17.2` if you need the previous Frigate release.

Pull it with:

```bash
docker pull msorenss79/hailo-frigate-h10:0.18.0-beta2
```
...
```yaml
frigate:
   image: msorenss79/hailo-frigate-h10:0.18.0-beta2
   pull_policy: always
